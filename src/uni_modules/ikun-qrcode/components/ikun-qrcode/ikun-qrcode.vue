...
<template>
	<view class="qr-wrapper" :style="{ width:`${width}${unit}`,height:`${height}${unit}` }">
		...
		<view v-else v-for="(row, rowI) in modules" :key="rowI" class="qr-content">
	...
	export default {
		...
		methods: {
			init() {
				...
				this.per = this.width / this.modules.length
				console.log("init", this.per)
			}
		}
	}
</script>
